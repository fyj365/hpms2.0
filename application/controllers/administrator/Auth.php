	...
	*/
	public function login()
	{
		...
		if ($this->form_validation->run()) {
			 ...
			 $row = $this->aauth->login($username, $password ,$remember);
            
            if ($row) {
		             if ($this->aauth->is_member(2,$row->id)) //if it's staff group
					 {
					 	redirect('administrator/voucher_line_tmp/add_register');
					 } 
					else if ($this->aauth->is_member(6,$row->id)) { //if its manger

					   redirect('administrator/voucher_line/index_3');

					}
					else if ($this->aauth->is_member(5,$row->id)) { //if its IT group

						 redirect('administrator/dashboard');
					  }
					else if ($this->aauth->is_member(1,$row->id)) { 
