...
class Crud extends Admin	
{
	...
	public function save_crud()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($this->input->post('save_type') == 'stay') {
				...
				$this->response['message'] = 'Your data has been successfully saved into the database. '.anchor('administrator/crud', ' Go back to list').' or '.anchor('administrator/'.$this->input->post('table_name'), ' View');
			} else {
				set_message('Your data has been successfully saved into the database. '.anchor('administrator/'.$this->input->post('table_name'), ' View'), 'success');
        		$this->response['success'] = true;
	...
	*/
	public function delete($id)
	{
		...
		if ($remove) {
            set_message('Crud has been deleted.', 'success');
		} else {
            set_message('Error delete crud.', 'error');
		}
